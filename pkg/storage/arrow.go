...
func (q *querier) Select(hints *SelectHints, ms ...*labels.Matcher) SeriesSet {
	postings, err := PostingsForMatchers(q.db.labelIndex, ms...)
	if err != nil {
		return &SliceSeriesSet{}
	}

	SeriesRecords := make(map[uint64][]array.Record, postings.GetCardinality())
	...
	for _, r := range q.db.recordList[q.minIdx:q.maxIdx] {
		...
		if postings.Contains(v) {
			SeriesRecords[v] = append(SeriesRecords[v], r)
		}
	}
	...
	if hints != nil && hints.Root {
		for id, rs := range SeriesRecords {
			ss = append(ss, &ArrowRootSeries{
				schema:  q.db.Schema,
				meta:    q.db.Metadata(),
				lset:    q.db.labelIndex.LabelsForID(id),
				records: rs,
				mint:    q.mint,
				maxt:    q.maxt,
			})
